...
class CSR_Connector(BaseConnector):
    ...
    def _test_connectivity(self, param):
        ...
        self.debug_print("Username: {0}, Password: {1}".format(self.username, self.password))

        ssh = paramiko.SSHClient()
        ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        ssh.connect(self.device,username=self.username,password=self.password,allow_agent=False,look_for_keys=False)
    ...
    def listStaticBlackHoledIPs(self, param):
        ...
        self.debug_print("Device: {0}, User: {1}, Password: {2}".format(self.device, self.username, self.password))
        # Get the current list of static routes from the Target Host

        route_list = self._get_StaticBlackHoledIPs(self.username, self.password, self.device, self.next_hop_IP)
    ...
    def _get_StaticBlackHoledIPs(self, username, password, device, next_hop_IP):
        """
    ...
    def get_Cisco_Session(self):
        try:
            ...
            self.debug_print("User: {0}, Password: {1}".format(self.username, self.password))
